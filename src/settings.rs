...
impl Settings {
    ...
    pub fn update(&mut self, setting: &str, value_str: &str) {
        match setting.as_ref() {
            ...
            "time_per_move" => self.time_per_move = value_str.parse::<i32>().unwrap(),
            ...
            "field_width"   => self.field_width = value_str.parse::<i32>().unwrap(),
            "field_height"  => self.field_height = value_str.parse::<i32>().unwrap(),
            "max_rounds"    => self.max_rounds = value_str.parse::<i32>().unwrap(),
            _ => {}
        }
    }
}